...
$$ 
Frpm Handout 3, we know the asymptotic distribution of $\hat{\beta}$ is normal, even if the (conditional) finite sample distribution of $u$ is not. Applying the Delta Method (see Handout 2) to this result, 
...
$$ 
Note, this statement is only true under $H_0$, while the former is
true always (under CLRM assumptions).
...
\end{aligned}
$$
For asymmetric distributions, this is not the case, and you must write the two rejection case separately. 
...
$$
The T-statistic can be rewritten as the Z-statistic (which had a standard normal distribution under $H_0$) multiplied by the square-root of the ratio of $s^2$ and $\sigma^2$. Consider the distribution of this ratio:

$$
  \frac{s^2}{\sigma^2} = \frac{1}{n-k} \frac{\hat{u}'\hat{u}}{\sigma^2}
$$
We know that $\hat{u} = M_XY = M_X u$, where $M_X$ is idempotent and symmetric. Therefore,  
...
$$
Thus, under the $H_0$, the T-statistic's distribution can be described by the ratio of two distributions,
...
R\hat{\beta}|X \sim N(R\beta,\sigma^2 R(X'X)^{-1}R')
$$ 
This statement is true always, while **under $H_0: R\beta = r$**, 
...
And the model includes a constant (i.e. $X_1 = \mathbf{1}$). Then, the restricted model amounts to a regression of $Y$ on a constant.

$$
  RSS_R = (Y-\bar{Y})'(Y-\bar{Y}) = TSS
$$

The F-statistic can be written in terms of $R^2$ (of the unrestricted model), 
