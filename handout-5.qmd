...
$$
\hat{\beta}^{GLS} = \underset{b}{\arg \max} \quad (Y-Xb)'\Sigma^{-1}(Y-Xb)
$$
which can be written as, 

$$
\hat{\beta}^{GLS} = \underset{b}{\arg \max} \quad (Y^+-X^+b)'(Y^+-X^+b)
$$
...
$$
 (I_n\otimes M_\ell)Y = \tilde{Y} = (I_n\otimes M_\ell)X\beta + \varepsilon = \tilde{X}\beta + \varepsilon
$$
...
$$
E[\Delta \varepsilon_{is}\Delta \varepsilon_{it}] =\begin{cases}E[(\varepsilon_{is}-\varepsilon_{is-1})(\varepsilon_{it}-\varepsilon_{it-1})] = 2\sigma^2_{\varepsilon} \quad \text{for}\;s=t \\
